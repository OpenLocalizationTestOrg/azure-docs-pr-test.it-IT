...
Se si dispone di un Provider multi-Factor Authentication o un'autenticazione a più fattori di Azure, Azure AD Premium o Enterprise Mobility + licenza di sicurezza, utilizza questa pagina di impostazioni servizio di autenticazione a più fattori di metodo tooaccess hello.
...
2. tooreport illecito, immettere codice illecito hello (valore predefinito è 0) e quindi hello simbolo #. Verrà segnalato che è stato inviato un avviso di illecito.
...
3. In selezionare superiore hello **provider multi-Factor Authentication**. Viene visualizzato un elenco dei provider di Multi-Factor Authentication.
...
7. Fare clic su **Run**. Verrà visualizzato un report degli avvisi di illecito. Fare clic su **esportare tooCSV** se si desidera report hello tooexport.
...
6. Immettere nome utente di hello, hello numero di secondi di bypass hello verrà esiste e hello motivo bypass hello. quindi fare clic su **Bypass**.
...
6. Specificare l'intervallo di date hello che si desidera tooview nel report hello. È anche possibile specificare i nomi utente, i numeri di telefono e hello lo stato utente.
7. Fare clic su **Run**. Verrà visualizzato un report dei bypass. Fare clic su **esportare tooCSV** se si desidera report hello tooexport.

## <a name="custom-voice-messages"></a>Messaggi vocali personalizzati
I messaggi vocali personalizzati consentono di toouse registrazioni o personali greetings per la verifica in due passaggi. Possono essere usati nei record di addizione tooor tooreplace hello Microsoft.

Prima di iniziare a tenere hello segue:

* formati di file Hello supportato sono. wav e. mp3.
* limite delle dimensioni file Hello è 5 MB.
* I messaggi di autenticazione devono avere una durata inferiore a 20 secondi. Qualsiasi elemento è più lungo rispetto a ciò potrebbe causare hello verifica toofail perché l'utente hello potrebbe non rispondere prima hello messaggio al termine, causando hello verifica tootime out.
...
10. Una volta completata l'operazione, un messaggio di conferma che è stato caricato correttamente il file hello.

messaggio hello tooturn in per gli utenti:
...
La memorizzazione nella cache consente tooset un periodo di tempo specifico in modo che i tentativi di autenticazione successivi entro tale periodo di tempo esito positivo automaticamente. Questo viene utilizzato principalmente per i sistemi locali, ad esempio VPN inviano più richieste di verifica mentre prima richiesta hello è ancora in corso. In questo modo hello richieste successive toosucceed automaticamente una volta terminato utente hello verifica prima di hello in corso. 
...
4. Pagina memorizzazione nella cache fare clic su Configura hello **nuova Cache**.
...
Questo bypass può essere usato solo all'interno della rete Intranet di un'azienda. Ad esempio, se selezionato gli utenti federati tutti e un utente accede dalla rete intranet della società hello esterno, tale utente tooauthenticate utilizzando la verifica in due passaggi anche se l'utente di hello presenta un'attestazione ADFS 
...
Se gli indirizzi IP attendibili è abilitato, verifica in due passaggi è *non* richiesti per i flussi dei browser e le password dell'app sono *non* necessari per le app rich client meno recenti, purché hello utente non è già stato creato un password dell'app. Se si inizia a usare una password dell'app, questa rimane obbligatoria. 
...
3. Nella pagina Impostazioni servizio hello in indirizzi IP attendibili, sono disponibili due opzioni:
   ...
   * **Per le richieste degli utenti federati originate dalla intranet dell'utente** : casella di controllo hello. Tutti gli utenti che accedono dalla rete aziendale hello Ignora verifica in due passaggi usando un'attestazione rilasciata da ADFS.
...
esempio Hello è un elenco di importanti delle operazioni che è necessario conoscere le password di app.

* Le password di App è necessario solo toobe immesso una sola volta per ogni app. Gli utenti non sono a tenerne traccia tookeep e immetterle ogni volta.
* la password effettiva Hello viene generata automaticamente e non viene fornita dall'utente hello. In questo modo la password generata automaticamente hello è più difficile per un utente malintenzionato tooguess ed è più sicura.
...
* Dopo aver abilitato Multi-factor Authentication nell’account di un utente, le password di app possono essere utilizzate con la maggior parte dei client non basati su browser come Outlook e Lync, ma non è possibile eseguire azioni amministrative usando password di app tramite applicazioni non basate su browser come Windows PowerShell, anche se tale utente dispone di un account di amministrazione.  Assicurarsi di creare un account di servizio con gli script di PowerShell toorun una password complessa e non abilitare tale account per la verifica in due passaggi.

> [!WARNING]
> Le password di app non funzionano in ambienti ibridi dove i client comunicano con endpoint di individuazione automatica sia locali che nel cloud. In questo modo le password del dominio sono necessari tooauthenticate locale e le password di app sono necessari tooauthenticate cloud hello.

### <a name="naming-guidance-for-app-passwords"></a>Indicazioni per la denominazione delle password dell'app
I nomi delle password di App deve riflettere dispositivo hello in cui vengono utilizzati. Ad esempio, se si usa un portatile con app non basate su browser come Outlook, Word ed Excel, è sufficiente creare una sola password dell'app denominata Portatile e usarla in queste applicazioni. Quindi, creare un'altra password di app denominata Desktop per hello stesse applicazioni nel computer desktop. 
...
* Alcune architetture avanzate possono richiedere una combinazione di nome utente e password dell'organizzazione e password dell'app quando si usa la verifica in due passaggi con i client, a seconda della posizione in cui viene eseguita l'autenticazione. Per i client che si autenticano con un'infrastruttura locale, verranno usati un nome utente e una password dell'organizzazione. Per i client che eseguono l'autenticazione con Azure AD, utilizzare una password di app hello.
  ...
  Si supponga, ad esempio, che si dispone di un'architettura costituita dai seguenti hello:
  ...
  * Si usa Azure Multi-Factor Authentication
  ...
  In questi casi, è necessario eseguire il seguente hello:
...
Per impostazione predefinita, gli utenti non possono creare password dell'app. Questa funzionalità deve essere abilitata. gli utenti tooallow hello le password di app toocreate possibilità, usare hello seguente procedura:
...
Gli utenti possono inoltre creare password di app dopo la registrazione modificando le impostazioni nel portale di Office 365 portal o hello Azure hello. Per altre informazioni e procedure dettagliate per gli utenti, vedere [Che cosa sono le password per le app in Azure Multi-Factor Authentication](./end-user/multi-factor-authentication-end-user-app-passwords.md).
...
La memorizzazione di Multi-Factor Authentication per i dispositivi e i browser che gli utenti considerano attendibili è una funzionalità disponibile gratuitamente per tutti gli utenti di MFA. Consente opzione hello agli utenti di toogive tooby passaggio autenticazione a più fattori per un numero prefissato di giorni dopo l'esecuzione di una corretta Accedi con l'autenticazione a più fattori. Questo può migliorare l'usabilità, riducendo il numero di hello di volte in cui un utente può eseguire la verifica nel hello stesso dispositivo.

Tuttavia, se un dispositivo o un account viene compromesso, la memorizzazione di MFA per i dispositivi attendibili può influire sulla sicurezza. Se viene compromesso un account aziendale o un dispositivo attendibile viene smarrito o rubato, è necessario [ripristinare Multi-Factor Authentication su tutti i dispositivi](multi-factor-authentication-manage-users-and-devices.md#restore-mfa-on-all-remembered-devices-for-a-user). L'azione revoca stato hello attendibile da tutti i dispositivi e utente di hello è di nuovo la verifica in due passaggi tooperform obbligatorio. È anche possibile indicare il toorestore utenti autenticazione a più fattori con i propri dispositivi con le istruzioni di hello in [gestire le impostazioni per la verifica in due passaggi](./end-user/multi-factor-authentication-end-user-manage-settings.md#require-two-step-verification-again-on-a-device-youve-marked-as-trusted)
...
Hello "non chiedere più per **X** giorni" non è visualizzata la casella di controllo per le app non basate su browser, supportano l'autenticazione moderna o meno. Queste app usano token di aggiornamento che forniscono nuovi token di accesso ogni ora. Quando un token di aggiornamento viene convalidato, verifica di Azure AD che hello ultima verifica in due passaggi fase eseguita era entro il numero di hello configurato di giorni. 

Pertanto, ricordare di autenticazione a più fattori nei dispositivi attendibili riduce il numero di hello di autenticazioni per le app web (che in genere richiesta ogni volta), ma aumenta hello numero di autenticazioni per i client modern Authentication (che richiede in genere ogni 90 giorni).
...
È possibile scegliere quali metodi di verifica mettere a disposizione degli utenti. tabella Hello riportata di seguito fornisce una breve panoramica di ogni metodo.
...
| Testo messaggio toophone |Invia un messaggio di testo contenente un codice di verifica. utente Hello è di tipo messaggio di testo di toohello tooeither richiesta risposta con codice di verifica hello o un codice di verifica tooenter hello hello sign nell'interfaccia. |
