...
In questo articolo viene illustrata l'hello passaggi toodeploy le applicazioni SAP in macchine virtuali (VM) in Azure, incluse le opzioni di distribuzione alternativi e risoluzione dei problemi. In questo articolo si basa sulle informazioni hello [macchine virtuali di Azure la pianificazione e implementazione di SAP NetWeaver][planning-guide]. Inoltre, integra la documentazione di installazione di SAP e note su SAP, che sono le risorse primarie hello per l'installazione e distribuzione del software SAP.

## <a name="prerequisites"></a>Prerequisiti
La configurazione di una macchina virtuale di Azure per la distribuzione del software SAP richiede più passaggi e coinvolge diverse risorse. Prima di iniziare, assicurarsi che siano soddisfatti i prerequisiti hello per l'installazione del software SAP in macchine virtuali in Azure.
 ...
 * **Tipo di disco VM**: selezionare il tipo di disco hello del disco hello del sistema operativo. Se si desidera toouse archiviazione Premium per i dischi dati, è consigliabile per disco del sistema operativo di hello anche usando l'archiviazione Premium.
...
3. **Impostazioni**:
  * **Archiviazione**
    * **Tipo di disco**: selezionare il tipo di disco hello del disco hello del sistema operativo. Se si desidera toouse archiviazione Premium per i dischi dati, è consigliabile per disco del sistema operativo di hello anche usando l'archiviazione Premium.
    * **Utilizzare dischi gestiti**: se si desidera toouse gestiti dischi, selezionare Sì. Per ulteriori informazioni sui dischi gestiti, vedere il capitolo [dischi gestiti] [ planning-guide-managed-disks] nella Guida alla pianificazione di hello.
  ...
  * **Estensioni**: È possibile installare le estensioni di macchina virtuale aggiungendo toohello distribuzione. Non è necessario tooadd estensioni in questo passaggio. estensioni di Hello necessari per il supporto SAP sono installate in un secondo momento. Vedere il capitolo [Configura hello Azure Enhanced Monitoring Extension per SAP] [ deployment-guide-4.5] in questa Guida.
  * **Disponibilità elevata**: selezionare un set di disponibilità o immettere i parametri di hello toocreate disponibilità di una nuova impostato. Per altre informazioni, vedere [Set di disponibilità di Azure][planning-guide-3.2.3].
  * **Monitoraggio**
    * **Diagnostica di avvio**: è possibile selezionare **Disabilita** per la diagnostica di avvio.
    * **Diagnostica del sistema operativo guest**: è possibile selezionare **Disabilita** per il monitoraggio della diagnostica.
...
* [**Modello per configurazione a due livelli (una sola macchina virtuale) - Managed Disks** (sap-2-tier-marketplace-image-md)][sap-templates-2-tier-marketplace-image-md]
  ...
  toocreate un sistema a due livelli utilizzando solo una macchina virtuale e i dischi gestiti, utilizzare questo modello.
...
* [**Modello per configurazione a tre livelli (più macchine virtuali) - Managed Disks** (sap-3-tier-marketplace-image-md)][sap-templates-3-tier-marketplace-image-md]
  ...
  toocreate un sistema a tre livelli con più macchine virtuali e dischi gestiti, utilizzare questo modello.
...
2. **Impostazioni**:
  ...
  * **Tipo di sistema operativo**: sistema operativo hello desiderato toodeploy, ad esempio, Windows Server 2012 R2, SUSE Linux Enterprise Server 12 (SLES 12), Red Hat Enterprise Linux 7.2 (RHEL 7.2) o Oracle Linux 7.2.

    visualizzazione elenco Hello non mostra che tutti i sistemi operativi supportati. Per altre informazioni sui sistemi operativi supportati per la distribuzione di software SAP, vedere la nota SAP [1928533].
...
Se la distribuzione di Azure viene connesso tooan istanza locale di Active Directory o DNS tramite una connessione VPN di Azure site-to-site o ExpressRoute (detta *cross-premise* in [pianificazione macchine virtuali di Azure e sull'implementazione di SAP NetWeaver][planning-guide]), è previsto che hello macchina virtuale è aggiunta a un dominio locale. Per ulteriori informazioni su considerazioni per questa attività, vedere [aggiunta a un dominio locale VM tooan (solo Windows)][deployment-guide-4.3].

#### <a name="ec323ac3-1de9-4c3a-b770-4ff701def65b"></a>Configurare il monitoraggio
SAP supporta la configurazione dell'ambiente, hello Azure Monitoring Extension per SAP come descritto in che toobe [Configura hello Azure Enhanced Monitoring Extension per SAP][deployment-guide-4.5]. Verifica dei prerequisiti di hello per il monitoraggio SAP e le versioni minime richieste del Kernel SAP e dell'agente Host SAP, in risorse hello elencate in [risorse SAP][deployment-guide-2.2].
...
#### <a name="create-a-virtual-machine-by-using-hello-azure-portal"></a>Creare una macchina virtuale utilizzando hello portale di Azure
toocreate modo più semplice di Hello una nuova macchina virtuale da un'immagine del disco gestito è tramite hello portale di Azure. Per ulteriori informazioni sulla modalità di lettura un'immagine disco gestire di toocreate [acquisire un'immagine gestita di una macchina virtuale generalizzata in Azure](https://docs.microsoft.com/azure/virtual-machines/windows/capture-image-resource)

1.  Andare troppo<https://ms.portal.azure.com/#blade/HubsExtension/Resources/resourceType/Microsoft.Compute%2Fimages>. In alternativa, nel menu del portale di Azure hello, selezionare **immagini**.
2.  Immagine del disco gestito hello selezionare toodeploy desiderato e fare clic su **crea macchina virtuale**
...
3. **Impostazioni**:
  ...
  * **Rete**
    ...
    * **Indirizzo IP pubblico**: selezionare hello indirizzo IP pubblico che si desidera toouse o immettere i parametri toocreate un nuovo indirizzo IP pubblico. È possibile utilizzare un tooaccess di indirizzo IP pubblico della macchina virtuale su hello Internet. Assicurarsi che anche creare una macchina di virtuale di rete sicurezza gruppo toohelp accesso sicuro tooyour.
    * **Gruppo di sicurezza di rete**: per altre informazioni, vedere [Controllare il flusso del traffico di rete con i gruppi di sicurezza di rete][virtual-networks-nsg].
  * **Estensioni**: È possibile installare le estensioni di macchina virtuale aggiungendo toohello distribuzione. Non è necessario tooadd estensione in questo passaggio. estensioni di Hello necessari per il supporto SAP sono installate in un secondo momento. Vedere il capitolo [Configura hello Azure Enhanced Monitoring Extension per SAP] [ deployment-guide-4.5] in questa Guida.
  * **Disponibilità elevata**: selezionare un set di disponibilità o immettere i parametri di hello toocreate disponibilità di una nuova impostato. Per altre informazioni, vedere [Set di disponibilità di Azure][planning-guide-3.2.3].
  * **Monitoraggio**
    * **Diagnostica di avvio**: è possibile selezionare **Disabilita** per la diagnostica di avvio.
    * **Diagnostica del sistema operativo guest**: è possibile selezionare **Disabilita** per il monitoraggio della diagnostica.
...
* [**Modello per configurazione a due livelli (una sola macchina virtuale) - Immagine di disco gestito** (sap-2-tier-user-image-md)][sap-templates-2-tier-user-image-md]
  ...
  toocreate un sistema a due livelli utilizzando una sola macchina virtuale e un'immagine del disco gestito, utilizzare questo modello.
...
* [**Modello per configurazione a tre livelli (più macchine virtuali) - Immagine di disco gestito** (sap-3-tier-user-image-md)][sap-templates-3-tier-user-image-md]
  ...
  toocreate un sistema a tre livelli con più macchine virtuali o la propria immagine del sistema operativo e un'immagine del disco gestito, utilizzare questo modello.
...
2. **Impostazioni**:
  ...
  * **Tipo di archiviazione** (solo modello a due livelli): tipo di archiviazione toouse hello.
    ...
    Per i sistemi più grandi, è consigliabile usare Archiviazione Premium di Azure. Per ulteriori informazioni sui tipi di archiviazione, vedere hello seguenti risorse:
      ...
      * [Archiviazione Premium: archiviazione ad alte prestazioni per carichi di lavoro delle macchine virtuali di Azure][storage-premium-storage-preview-portal]
      * [Introduzione tooMicrosoft di archiviazione di Azure][storage-introduction]
  * **Immagine utente URI VHD** (solo modello di immagine disco non gestito): hello URI dell'immagine del sistema operativo privata hello disco rigido virtuale, ad esempio https://&lt;accountname >.blob.core.windows.net/vhds/userimage.vhd.
  * **Account di archiviazione immagine utente** (solo modello di immagine disco non gestito): nome hello dell'immagine del sistema operativo privata hello memorizzazione, ad esempio, account di archiviazione hello &lt;accountname > in https://&lt;accountname >. BLOB.Core.Windows.NET/vhds/userimage.vhd.
  * **userImageId** (solo modello di immagine disco gestito): Id dell'immagine del disco gestito da toouse hello
...
#### <a name="join-a-domain-windows-only"></a>Aggiungere a un dominio (solo per Windows)
Se la distribuzione di Azure viene connesso tooan istanza locale di Active Directory o DNS tramite una connessione VPN di Azure site-to-site o ExpressRoute di Azure (detta *cross-premise* in [macchine virtuali di Azure pianificazione e implementazione di SAP NetWeaver][planning-guide]), è previsto che hello macchina virtuale è aggiunta a un dominio locale. Per ulteriori informazioni su considerazioni per questo passaggio, vedere [aggiunta a un dominio locale VM tooan (solo Windows)][deployment-guide-4.3].
...
In questo scenario, hello agente VM è **non** installato automaticamente durante la distribuzione. Poiché l'agente VM hello e hello Azure Enhanced Monitoring Extension per SAP sono necessari toorun SAP NetWeaver in Azure, è necessario toodownload, installare e attivare entrambi i componenti manualmente dopo la creazione di macchine virtuali hello.
...
Se il disco hello già caricato e definito in Azure (vedere [macchine virtuali di Azure la pianificazione e implementazione di SAP NetWeaver][planning-guide]), effettuare le attività di hello descritti in hello accanto sezioni.
...
* [**Modello per configurazione a due livelli (una sola macchina virtuale) - Disco gestito** (sap-2-tier-user-disk-md)][sap-templates-2-tier-os-disk-md]
  ...
  toocreate un sistema a due livelli utilizzando una sola macchina virtuale e un disco gestito, utilizzare questo modello.
...
2. **Impostazioni**:
  ...
  * **Tipo di archiviazione** (solo modello a due livelli): tipo di archiviazione toouse hello.
    ...
    Per i sistemi più grandi, è consigliabile usare Archiviazione Premium di Azure. Per ulteriori informazioni sui tipi di archiviazione, vedere hello seguenti risorse:
      ...
      * [Archiviazione Premium: archiviazione ad alte prestazioni per carichi di lavoro delle macchine virtuali di Azure][storage-premium-storage-preview-portal]
      * [Introduzione tooMicrosoft di archiviazione di Azure][storage-introduction]
  * **URI VHD su disco SO** (solo modello di disco non gestito): hello URI del disco del sistema operativo privato hello, ad esempio https://&lt;accountname >.blob.core.windows.net/vhds/osdisk.vhd.
  * **Id disco gestito su disco SO** (solo modello di disco gestito): hello Id di hello disco gestiti del sistema operativo del disco, /subscriptions/92d102f7-81a5-4df7-9877-54987ba97dd9/resourceGroups/group/providers/Microsoft.Compute/disks/WIN
  * **Subnet nuova o esistente**: determina se vengono create una nuova rete virtuale e una nuova subnet o viene usata una subnet esistente. Se si dispone già di una rete virtuale è una rete locale tooyour connesso, selezionare **esistente**.
...
* Montare tooyour dischi dati aggiuntivi macchina virtuale di Azure o si rimuove un disco dati. In questo scenario, aggiornare la raccolta di hello dei dati relativi alla memoria. Modifica della configurazione mediante l'aggiunta o eliminazione di endpoint o assegnando IP indirizzi tooa VM non influiscono sulla hello Monitoraggio configurazione.
...
* **Oracle Linux**
  ...
  Non esistono repository per Oracle Linux in Azure. È necessario tooconfigure proprio repository per Oracle Linux o usare repository pubblici hello.
...
2. Eseguire hello seguente cmdlet di PowerShell.
    Per un elenco degli ambienti disponibili, eseguire `commandlet Get-AzureRmEnvironment`. Se si desidera toouse globale Azure, l'ambiente è **cloud**. Per Azure in Cina, selezionare **AzureChinaCloud**.
...
* Conferma che è stato configurato il monitoraggio per il disco del sistema operativo hello e tutti i dischi dati aggiuntivi.
...
1. Installare Azure CLI 1.0, come descritto in [installazione hello Azure CLI 1.0][azure-cli].
...
2.  Eseguire hello seguente cmdlet di PowerShell. Per un elenco degli ambienti disponibili, eseguire i cmdlet di hello `Get-AzureRmEnvironment`. Selezionare di Azure, globale toouse hello **cloud** ambiente. Per Azure in Cina, selezionare **AzureChinaCloud**.
  ```powershell
  $env = Get-AzureRmEnvironment -Name <name of hello environment>
  Login-AzureRmAccount -Environment $env
  Set-AzureRmContext -SubscriptionName <subscription name>
