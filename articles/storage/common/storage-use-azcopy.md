...
Si noti che se la cartella hello `C:\myfolder` non esiste, AzCopy crea il download e `abc.txt ` nella nuova cartella hello.
...
Dopo un'operazione di download hello hello directory `C:\myfolder` include hello i seguenti file:
...
Dopo un'operazione di download hello hello cartella `C:\myfolder` include hello i seguenti file:

    C:\myfolder\abc.txt
    C:\myfolder\abc1.txt
    C:\myfolder\abc2.txt

prefisso Hello applica toohello directory virtuale, che costituisce hello prima parte del nome blob hello. Nell'esempio hello illustrato in precedenza, la directory virtuale hello non corrisponde prefisso specificato hello, in modo che non è stato scaricato. Inoltre, se hello opzione `\S` non viene specificato, AzCopy non scarica tutti i BLOB.
...
Se hello specificato una directory virtuale non esiste, AzCopy carica hello file tooinclude hello directory virtuale nel nome (*ad esempio*, `vd/abc.txt` nell'esempio hello sopra).
...
Specifica l'opzione `/S` contenuto hello caricamenti di hello specificato directory tooBlob archiviazione in modo ricorsivo, vale a dire che tutte le sottocartelle e i file vengono caricati anche. Ad esempio, si supponga che segue hello file si trovano nella cartella `C:\myfolder`:
...
Se non si specifica l'opzione `/S`, AzCopy non carica i file in modo ricorsivo. Dopo un'operazione di caricamento hello contenitore hello include hello i seguenti file:
...
Se si specifica `/SetContentType` senza un valore, AzCopy imposta ogni blob o il tipo di contenuto del file in base a tooits estensione di file.
...
Dopo un'operazione di copia hello contenitore di destinazione hello include blob hello e i relativi snapshot. Se il blob di hello esempio hello precedente contiene due snapshot, contenitore hello include seguente hello blob e snapshot:
...
Hello `/SyncCopy` opzione assicura che l'operazione di copia hello Ottiene velocità coerente. AzCopy esegue copia sincrono hello scaricando BLOB hello toocopy da hello specificato memoria toolocal di origine e quindi caricarli toohello destinazione di archiviazione Blob.
...
È possibile specificare hello `/SyncCopy` opzione toocopy dati da archiviazione di File tooFile, archiviazione, dall'archiviazione di File tooBlob, archiviazione e dall'archiviazione Blob tooFile archiviazione in modo sincrono, AzCopy esegue questa operazione di download di memoria di toolocal hello origine dati e di caricarlo toodestination nuovamente. Vengono applicati i costi in uscita standard.
...
indice di intervalli di chiavi di partizione Hello è 0 se l'utente hello non specifica l'opzione `/PKRS`.
...
Quando si specifica il formato di payload di hello CSV, AzCopy genera anche un file di schema con estensione `.schema.csv` per ogni file di dati.
...
AzCopy inizia entità tooexport operazioni simultanee quando l'utente hello specifica l'opzione `/PKRS`. Ogni operazione esporta un intervallo di chiavi di partizione.
...
AzCopy genera un file di dati JSON in un contenitore blob hello con convenzione di denominazione seguente:
...
Si noti che, quando si esportano tabelle tooblobs, AzCopy Scarica file di dati temporanei toolocal entità tabella hello e quindi carica tali blob toohello di entità. Questi file di dati temporanei vengono inseriti nella cartella di file hello giornale di registrazione con il percorso predefinito di hello "<code>%LocalAppData%\Microsoft\Azure\AzCopy</code>", è possibile specificare l'opzione/toochange [cartella di file journal] z: hello percorso della cartella file journal e pertanto modificare percorso dei file di dati temporanei hello. dati temporanei, le dimensioni dei file viene stabilita delle entità di tabella dimensioni e hello che è specificato con /SplitSize opzione hello, anche se i file di dati temporanei hello nel disco locale viene eliminato immediatamente dopo che è stato Hello caricato toohello blob, assicurarsi di disporre locale sufficiente su disco toostore spazio questi file di dati temporanei, prima di essere eliminati.
...
AzCopy ha esito negativo se il parametro hello si divide in due righe, come illustrato di seguito per hello `/sourcekey` parametro:
...
Se hello origine risorsa è un contenitore di blob e viene fornita una chiave né una firma di accesso condiviso, il contenitore di blob hello viene letto tramite l'accesso anonimo.
...
### <a name="s"></a>/S
Specifica la modalità ricorsiva per le operazioni di copia. In modalità ricorsiva, AzCopy copia tutti i BLOB o i file che corrispondono a hello criterio di file specificato, inclusi quelli nelle sottocartelle.
...
Per impostazione predefinita, il file di log dettagliato hello denominato AzCopyVerbose.log in `%LocalAppData%\Microsoft\Azure\AzCopy`. Se si specifica un percorso del file esistente per questa opzione, log dettagliato hello è toothat aggiunto file.  
...
Se questa opzione non è specificata o viene specificato senza un percorso di cartella, AzCopy crea file journal hello nel percorso predefinito di hello, ovvero % LocalAppData%\Microsoft\Azure\AzCopy.
...
### <a name="xn"></a>/XN
Esclude una risorsa di origine più recente. Hello risorsa non verrà copiata se l'ora dell'ultima modifica dell'origine hello hello è hello uguale o più recente di destinazione.

**Applicabile a:** BLOB, file

### <a name="xo"></a>/XO
Esclude una risorsa di origine meno recente. Hello risorsa non verrà copiata se l'ora dell'ultima modifica dell'origine hello hello è hello uguale o più vecchi di destinazione.
...
Se questa opzione non è specificata, AzCopy Esporta file singolo tooa dati di tabella.

Se i dati di tabella hello esportata tooa blob, e hello esportato il file raggiunge il limite di 200 GB hello per le dimensioni del blob, AzCopy suddivide hello file esportato, anche se questa opzione non è specificata.
...
Questa opzione è facoltativa durante l'operazione di esportazione hello, AzCopy genera un file manifesto con il nome predefinito se non si specifica questa opzione.
...
Se si specifica questa opzione senza un valore, AzCopy imposta ogni blob o il tipo di contenuto del file in base a tooits estensione di file.
...
Per la proprietà "AzureStorageUseV1MD5" • True: valore predefinito di hello, AzCopy utilizza implementazione .NET MD5.
...
* [Come toouse archiviazione Blob da .NET](../blobs/storage-dotnet-how-to-use-blobs.md)
* [Come toouse archiviazione di File da .NET](../storage-dotnet-how-to-use-files.md)
* [Come toouse archiviazione tabelle da .NET](../../cosmos-db/table-storage-how-to-use-dotnet.md)
