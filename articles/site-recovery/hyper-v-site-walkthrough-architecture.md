...
---
# <a name="step-1-review-hello-architecture-for-hyper-v-replication-tooazure"></a>Passaggio 1: Esaminare l'architettura di hello per Hyper-V replica tooAzure


Questo articolo vengono descritti i componenti di hello e processi per la replica di macchine virtuali di Hyper-V (che non sono gestiti da System Center VMM), in locale utilizzando hello tooAzure [Azure Site Recovery](site-recovery-overview.md) servizio.
...
Non è coinvolto un numero di componenti per la replica delle macchine virtuali Hyper-V tooAzure senza VMM.
...
**Hyper-V** | Gli host e i cluster di Hyper-V vengono raccolti nei siti Hyper-V. Hello Provider di Azure Site Recovery e servizi di ripristino dell'agente è installato in ogni host Hyper-V. | Hello Provider Orchestra la replica con il ripristino del sito su hello internet. agente di servizi di ripristino Hello gestisce la replica dei dati.<br/><br/> Le comunicazioni da hello Provider e agente hello sono protetto e crittografato. I dati replicati nell'archiviazione di Azure vengono anche crittografati.
...
4. È possibile monitorare il processo di hello in hello **processi** scheda.
...
Andare troppo[passaggio 2: esaminare i prerequisiti di distribuzione hello](hyper-v-site-walkthrough-prerequisites.md)
