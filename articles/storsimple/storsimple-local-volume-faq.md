...
Nei dispositivi che eseguono StorSimple 8000 Series Update 2. x, è possibile eseguire il provisioning di volumi aggiunti in locale fino too8 TB o i volumi a livelli di too200 TB sul dispositivo hello 8100. Nel dispositivo 8600 hello più grande, è possibile eseguire il provisioning di volumi aggiunti in locale fino too20 TB o a più livelli di too500 TB.   

**D.** Aggiornato di recente my tooUpdate dispositivo 8100 2.0 e quando si tenta di toocreate un volume aggiunto in locale, dimensione massima disponibile di hello è solo 6 TB e non 8 TB. Perché non è possibile creare un volume di 8 TB?

**Oggetto** se il dispositivo sta eseguendo l'aggiornamento 2.0, è possibile eseguire il provisioning di volumi aggiunti in locale fino too8 TB o i volumi a livelli di too200 TB in hello dispositivo 8100. Se il dispositivo ha già a livelli volumi, spazio hello disponibile per la creazione di un volume aggiunto in locale sarà proporzionalmente inferiore a questo limite massimo. Se, ad esempio, 100 TB di volumi a livelli è già stato eseguito il provisioning sul dispositivo 8100 (che rappresenta la metà di hello a più livelli capacità), quindi hello la dimensione massima di un volume locale che è possibile creare il dispositivo 8100 hello sarà ridotto too4 TB (circa la metà del numero massimo di Hello localmente bloccata capacità del volume).
...
* È possibile convertire i volumi toolocally bloccato volumi a livelli esistenti e questa conversione comporta il provisioning di spazio sul dispositivo hello per hello risultante volume aggiunto in locale (in aggiunta toobringing dati a più livelli [eventuali] di cloud hello). Questa è poi un'operazione di lunga durata che dipende dai fattori illustrati sopra. Si consiglia di eseguire il backup del tooconversion precedente di volumi esistenti come processo hello risulterà anche più lenta se i volumi esistenti non vengono sottoposti a backup. Il dispositivo può anche subire un calo delle prestazioni durante questo processo.

Ulteriori informazioni su come troppo[creare un volume aggiunto in locale](storsimple-manage-volumes-u2.md#add-a-volume)
...
È possibile visualizzare tali processi in hello **processi** pagina di hello servizio Azure StorSimple Manager. Hello processo attivamente elaborati viene continuamente aggiornato lo stato di avanzamento di tooreflect hello del provisioning di spazio. Hello rimanenti processi volume aggiunto in locale viene contrassegnato come in esecuzione, ma lo stato di avanzamento è bloccato e vengono prelevati in ordine di hello che vengono messe in coda.
...
Ulteriori informazioni su come troppo[convertire un volume aggiunto in locale tooa volume a livelli](storsimple-manage-volumes-u2.md#change-the-volume-type)
...
* Un avviso critico sarà generato toonotify di errore di conversione del volume hello. Ulteriori informazioni su [gli avvisi di volumi correlati toolocally bloccato](storsimple-manage-alerts.md#locally-pinned-volume-alerts)
...
**R.** Sì, Tuttavia, verrà clonate come un volume a livelli volume hello aggiunto in locale per impostazione predefinita. Ulteriori informazioni su come troppo[clonare un volume aggiunto in locale](storsimple-clone-volume-u2.md)
...
**R.** A seconda della versione di hello del software del dispositivo di destinazione hello, volumi aggiunti in locale verranno eseguiti il failover come:

* Aggiunto in locale se il dispositivo di destinazione hello è in esecuzione StorSimple 8000 series aggiornamento 2
* A più livelli se il dispositivo di destinazione hello è in esecuzione StorSimple 8000 serie aggiornamento 1. x
* A più livelli se il dispositivo di destinazione hello è appliance di cloud hello (aggiornamento di versione di software 2 o l'aggiornamento 1. x)
...
**R.** No, non è possibile modificare il tipo di volume hello durante un failover. Se viene eseguito il failover tooanother fisico dispositivo che esegue update serie StorSimple 8000 volumi hello 2, verrà eseguito il failover in base al tipo di volume hello archiviato nello snapshot hello. Quando si esegue il failover tooany altra versione del dispositivo, fare riferimento domanda toohello sopra nel tipo di volume hello dopo un failover.

**D.** È possibile failover un contenitore di volumi con appliance di cloud toohello volumi aggiunti in locale?

**R.** Sì, volumi Hello aggiunto in locale verranno eseguiti il failover come volumi a livelli. Sono disponibili altre informazioni sugli [failover e ripristino di emergenza dei volumi aggiunti in locale nelle diverse versioni](storsimple-device-failover-disaster-recovery.md#considerations-for-device-failover)
