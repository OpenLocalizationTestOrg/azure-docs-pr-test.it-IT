...
Data Factory di Azure supporta i seguenti tipi di formato hello:
...
Se si desidera che i file di testo hello tooparse o scrivono dati hello in formato testo, impostare hello `format` `type` proprietà troppo**TextFormat**. È inoltre possibile specificare i seguenti hello **facoltativo** proprietà hello `format` sezione. Vedere [esempio TextFormat](#textformat-example) sezione su come tooconfigure.

| Proprietà | Descrizione | Valori consentiti | Obbligatorio |
| --- | --- | --- | --- |
| columnDelimiter |carattere Hello usato tooseparate colonne in un file. È possibile prendere in considerazione toouse un carattere non stampabile raro che probabilmente non esiste nei dati: ad esempio, specificare "\u0001" che rappresenta l'avvio di intestazione (SOH). |È consentito un solo carattere. Hello **predefinito** valore **virgola (',')**. <br/><br/>toouse un carattere Unicode, vedere troppo[caratteri Unicode](https://en.wikipedia.org/wiki/List_of_Unicode_characters) tooget hello codice corrispondente per tale. |No |
...
Hello seguenti vengono illustrate alcune delle proprietà relative al formato hello per TextFormat.
...
troppo**importazione/esportazione di file JSON come-è in/da Azure Cosmos DB**, vedere [documenti JSON di importazione/esportazione](../articles/data-factory/data-factory-azure-documentdb-connector.md#importexport-json-documents) sezione nel connettore di Azure Cosmos DB hello con i dettagli.
...
Vedere di seguito due tipi di esempi di quando si copiano dati da file JSON e hello toonote punti generico:
...
e, per ogni record, si prevede di toowrite tooa oggetto JSON nel seguente formato:
```json
{
    "id": "1",
    "order": {
