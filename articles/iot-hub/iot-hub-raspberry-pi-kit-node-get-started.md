...
1. Installare tutti i pacchetti hello comando seguente. Include Azure IoT SDK per dispositivi, la libreria del sensore BME280 e libreria di Pi per i collegamenti.
   ...
   ```
   > [!NOTE] 
   Potrebbe richiedere diversi minuti toofinish questo processo di installazione a seconda della connessione di rete.
...
1. Aprire il file di configurazione hello eseguendo hello seguenti comandi:
   ...
   Questo file contiene due elementi configurabili. Hello prima uno è `interval`, che definisce l'intervallo di tempo hello (in millisecondi) tra i due messaggi che inviano toocloud. Hello secondo `simulatedData`, ovvero un valore booleano per se toouse sensore dati simulati o non.
...
Eseguire l'applicazione di esempio hello eseguendo hello comando seguente:
