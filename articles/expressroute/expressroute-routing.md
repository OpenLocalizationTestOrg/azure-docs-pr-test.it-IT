...
* Se viene usata una subnet /29, questa verrà divisa in due subnet /30. 
  * prima di Hello/30 subnet viene utilizzato per il collegamento primario di hello e hello /30 secondo subnet viene utilizzata per collegamento secondario hello.
  * Per ogni subnet hello /30, è necessario utilizzare hello primo indirizzo IP del subnet hello /30 sul router. Microsoft utilizza secondo indirizzo IP hello di hello /30 subnet tooset una sessione BGP.
...
Se si sceglie toouse a.b.c.d/29 tooset backup hello peering, verrà suddiviso in due /30 subnet. Nell'esempio hello riportato di seguito, verranno esaminate utilizzo subnet a.b.c.d/29 hello. 
...
* Se viene usata una subnet /29, questa verrà divisa in due subnet /30. 
  * Hello innanzitutto/30 subnet verrà utilizzato per collegamento primario hello e hello /30 secondo subnet viene utilizzata per collegamento secondario hello.
  * Per ogni subnet hello /30, è necessario utilizzare hello primo indirizzo IP del subnet hello /30 sul router. Microsoft utilizza secondo indirizzo IP hello di hello /30 subnet tooset una sessione BGP.
  * È necessario impostare entrambe le sessioni BGP per il nostro [SLA sulla disponibilità](https://azure.microsoft.com/support/legal/sla/) toobe valido.
...
È possibile scegliere toouse gli indirizzi IPv4 pubblici o privati per il peering privato. Microsoft offre un isolamento end-to-end del traffico, quindi la sovrapposizione degli indirizzi con altri clienti non si verifica in caso di peering privato. Questi indirizzi non sono tooInternet annunciato. 
...
Assicurarsi che l'indirizzo IP e come numero sono registrati tooyou in uno dei hello seguenti registri:
