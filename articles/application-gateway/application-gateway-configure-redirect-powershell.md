---
title: reindirizzamento aaaConfigure per il Gateway applicazione Azure - PowerShell | Documenti Microsoft
description: Questa pagina fornisce il reindirizzamento tooconfigure scenari per il Gateway applicazione utilizzando PowerShell
...
---
# <a name="configure-redirection-on-application-gateway-with-powershell"></a><span data-ttu-id="b2707-103">Configurare il reindirizzamento nel gateway applicazione con PowerShell</span><span class="sxs-lookup"><span data-stu-id="b2707-103">Configure redirection on Application Gateway with PowerShell</span></span>

<span data-ttu-id="b2707-104">Gateway applicazione supporta il traffico di tooredirect possibilità hello in base a una configurazione definita.</span><span class="sxs-lookup"><span data-stu-id="b2707-104">Application gateway supports hello ability tooredirect traffic based on a defined configuration.</span></span> <span data-ttu-id="b2707-105">informazioni sul reindirizzamento in visita generale, toolearn [Panoramica di reindirizzamento Gateway applicazione](application-gateway-redirect-overview.md).</span><span class="sxs-lookup"><span data-stu-id="b2707-105">toolearn more about redirection in general visit, [Application Gateway redirect overview](application-gateway-redirect-overview.md).</span></span> <span data-ttu-id="b2707-106">In questo articolo vengono forniti esempi di reindirizzamento tooHTTPS HTTP, percorso di base reindirizza, reindirizzamento multisito e reindirizza tooexternal siti.</span><span class="sxs-lookup"><span data-stu-id="b2707-106">This article provides examples of HTTP tooHTTPS redirection, path based redirects, multi-site redirects, and redirects tooexternal sites.</span></span>

## <a name="http-toohttps-redirect-on-an-existing-application-gateway"></a><span data-ttu-id="b2707-107">Reindirizzamento HTTP tooHTTPS in caso di un gateway applicazione esistente</span><span class="sxs-lookup"><span data-stu-id="b2707-107">HTTP tooHTTPS redirect on an existing application gateway</span></span>

<span data-ttu-id="b2707-108">Hello seguente viene creato un nuovo listener HTTP nella porta 80 tooredirect toohello esistente listener delle richieste sulla porta 443.</span><span class="sxs-lookup"><span data-stu-id="b2707-108">hello following example creates a new HTTP listener on port 80 tooredirect requests toohello existing listener on port 443.</span></span>
...
<span data-ttu-id="b2707-109">Una volta gateway applicazione hello viene aggiornato, testare nuovo listener di hello sulla porta 80.</span><span class="sxs-lookup"><span data-stu-id="b2707-109">Once hello application gateway is updated, test hello new listener on port 80.</span></span>  <span data-ttu-id="b2707-110">Hello esempio seguente viene illustrato l'utilizzo `curl` test reindirizzamento hello.</span><span class="sxs-lookup"><span data-stu-id="b2707-110">hello following example shows using `curl` test hello redirection.</span></span>
...
## <a name="path-based-redirect"></a><span data-ttu-id="b2707-111">Reindirizzamento basato sul percorso</span><span class="sxs-lookup"><span data-stu-id="b2707-111">Path based redirect</span></span>

<span data-ttu-id="b2707-112">Hello di esempio seguente crea un nuovo listener e una regola di percorso basato su un gateway applicazione esistente che reindirizza solo il traffico soddisfa percorso url specifico hello.</span><span class="sxs-lookup"><span data-stu-id="b2707-112">hello following example creates a new listener and a path based rule on an existing application gateway that redirects only traffic meeting hello specific url path.</span></span>
...
## <a name="multi-site-redirect"></a><span data-ttu-id="b2707-113">Reindirizzamento multisito</span><span class="sxs-lookup"><span data-stu-id="b2707-113">Multi-site redirect</span></span>

<span data-ttu-id="b2707-114">Hello seguente esempio viene creato un nuovo gateway applicazione 2 multisito listener sulla porta 80.</span><span class="sxs-lookup"><span data-stu-id="b2707-114">hello following example creates a new application gateway with 2 multi-site listeners on port 80.</span></span> <span data-ttu-id="b2707-115">i listener di Hello sono per adatum.com e adatum.org. Traffico tooredirect viene creata una regola di reindirizzamento da adatum.org tooadatum.com. Configurazione aggiuntiva è necessaria per la configurazione di CNAME alias toohello applicazione gateway indirizzo IP pubblico, per ulteriori informazioni sulla delega il tooAzure di dominio DNS e creazione di CNAME visitano i record del dominio, [delegare a un dominio tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).</span><span class="sxs-lookup"><span data-stu-id="b2707-115">hello listeners are for adatum.com and adatum.org. A redirect rule is created tooredirect traffic from adatum.org tooadatum.com. Additional configuration is required for configuring CNAME aliases toohello application gateway public IP address, for more information on delegating your domain tooAzure DNS and creating CNAME records for your domain visit, [Delegate a domain tooAzure DNS](../dns/dns-delegate-domain-azure-dns.md).</span></span>
...
## <a name="redirect-tooan-external-site"></a><span data-ttu-id="b2707-116">Sito esterno tooan di reindirizzamento</span><span class="sxs-lookup"><span data-stu-id="b2707-116">Redirect tooan external site</span></span>

<span data-ttu-id="b2707-117">Hello esempio reindirizza il traffico in arrivo in hello applicazione gateway tooan sito Web esterno (bing.com).</span><span class="sxs-lookup"><span data-stu-id="b2707-117">hello following example redirects traffic coming into hello application gateway tooan external website (bing.com).</span></span> <span data-ttu-id="b2707-118">Quando si utilizza hello `TargetUrl` hello commutatore `IncludePath` e `IncludeQuery` opzioni non sono consentite.</span><span class="sxs-lookup"><span data-stu-id="b2707-118">When using hello `TargetUrl` switch hello `IncludePath` and `IncludeQuery` switches are not allowed.</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="b2707-119">Passaggi successivi</span><span class="sxs-lookup"><span data-stu-id="b2707-119">Next steps</span></span>

<span data-ttu-id="b2707-120">Visitare [configurare fine tooend SSL con il gateway applicazione con PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span><span class="sxs-lookup"><span data-stu-id="b2707-120">Visit [configure end tooend SSL with application gateway using PowerShell](application-gateway-end-to-end-ssl-powershell.md)</span></span>
