...
In questa esercitazione, è illustrato come toointegrate Symantec Web Security Service (WSS) con Azure Active Directory (Azure AD).

Integrazione del servizio di sicurezza Web Symantec (WSS) con Azure AD fornisce hello seguenti vantaggi:

- È possibile controllare in Azure AD che ha accesso tooSymantec Web Security Service (WSS)
- È possibile abilitare l'utenti tooautomatically get connesso tooSymantec Web Security Service (WSS) (Single Sign-On) con i propri account Azure AD
...
1. Aggiunta del servizio di sicurezza Web Symantec (WSS) dalla raccolta hello
...
4. Nella casella di ricerca hello, digitare **Symantec Web Security Service (WSS)**.
...
5. Nel riquadro dei risultati hello, selezionare **Symantec Web Security Service (WSS)**, quindi fare clic su **Aggiungi** pulsante applicazione hello tooadd.
...
3. **[Creazione di un utente di test del servizio di sicurezza Web Symantec (WSS)](#creating-a-symantec-web-security-service-wss-test-user)**  -toohave un equivalente di Britta Simon in Symantec Web Security Service (WSS) che è la rappresentazione toohello collegato Azure AD dell'utente.
...
3. In hello **URL e dominio del servizio (WSS) sicurezza Web Symantec** seguire hello alla procedura seguente:
    ...
    a. In hello **Sign-on URL** casella menzione hello url, che si desidera tooblock in base a criteri di blocco toohello di hello Symantec Web Security Service (WSS).  
    
    b. In hello **identificatore** casella di testo, digitare l'URL hello:`https://saml.threatpulse.net:8443/saml/saml_realm`

    > [!NOTE] 
    > Poiché questi non sono i valori reali, Aggiornare questi valori con hello effettivo URL di accesso e l'identificatore. Contatto [team di supporto Client di servizio di sicurezza Web Symantec (WSS)](https://www.symantec.com/contact-us) tooget questi valori. 
...
6. tooconfigure single sign-on sul **Symantec Web Security Service (WSS)** lato, è necessario hello toosend scaricato **Metadata XML** troppo[teamdisupportodiSymantecWebSecurityService(WSS)](https://www.symantec.com/contact-us).
...
In questa sezione viene creato un utente di nome Britta Simon in Symantec Web Security Service (WSS). Lavorare con [team di supporto del servizio di sicurezza Web Symantec (WSS)](https://www.symantec.com/contact-us) per aggiungere gli utenti di hello nella piattaforma di hello Symantec Web Security Service (WSS). Gli utenti devono essere creati e attivati prima di usare l'accesso Single Sign-On. Anche insieme hello dettagli utente è necessario toosend hello indirizzo IP pubblico del computer di hello da cui si sta tentando di un'applicazione hello tooaccess.
...
Quando si fa clic su riquadro Symantec Web Security Service (WSS) nel Pannello di accesso hello hello, si ottiene toohello reindirizzato il blocco di pagina per cui è stato configurato hello criteri di blocco.
