...
Quando si crea un Database di Azure per il server MySQL, si decide tre opzioni principali risorse hello tooconfigure allocate per il server. Queste scelte impatto sulle prestazioni di hello e la scalabilità del server di hello.
- Piano tariffario 
- Unità di calcolo
- Archiviazione (GB)
...
In un database di Azure per il server MySQL è possibile avere uno o più database. È possibile rifiutare un singolo database per ogni server tooutilize toocreate tutte le risorse di hello o creare più database tooshare risorse hello. 
...
- Informazioni su come troppo[monitoraggio e la scala di un Database di Azure per il server MySQL mediante Azure CLI](scripts/sample-scale-server.md)
