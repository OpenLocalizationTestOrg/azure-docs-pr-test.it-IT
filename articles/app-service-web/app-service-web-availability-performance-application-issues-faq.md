...
Per determinare se si sta usando la cache locale, verificare la scheda **Impostazioni dell'applicazione** del servizio app. Se si sta usando la cache locale, l'impostazione `WEBSITE_LOCAL_CACHE_OPTION` dell'app ha il valore `Always`. Per altre informazioni sulla cache locale, vedere [Panoramica della cache locale del servizio app di Azure](https://docs.microsoft.com/azure/app-service/app-service-local-cache).
