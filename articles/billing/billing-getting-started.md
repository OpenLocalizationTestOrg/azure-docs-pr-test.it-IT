...
Quando effettua l'iscrizione per Azure, esistono alcune operazioni, è possibile eseguire un'idea precisa la spesa tooget. In hello [portale di Azure](https://portal.azure.com/#blade/Microsoft_Azure_Billing/SubscriptionsBlade), quando si seleziona una sottoscrizione di hello, è possibile visualizzare la suddivisione di costo corrente e velocità. nonché [scaricare le fatture precedenti e i file dei dati dettagliati di uso](billing-download-azure-invoice-daily-usage-date.md). Se si desidera toogroup costi per le risorse utilizzate per progetti diversi o team, esaminare [tag risorsa](../azure-resource-manager/resource-group-using-tags.md). Se l'organizzazione dispone di un sistema che si preferisce toouse, estrarre hello [fatturazione API](billing-usage-rate-card-overview.md). 
...
Se la sottoscrizione è una versione di valutazione gratuita [Visual Studio](https://azure.microsoft.com/pricing/member-offers/msdn-benefits-details/), Azure in Open (AIO) o BizSpark, quindi apprendere [i limiti di spesa](#spending-limit) tooavoid con la sottoscrizione unexpectantly disabilitata. 
...
Estrarre hello [calcolatore dei costi](https://azure.microsoft.com/pricing/calculator/) e [costo totale di calcolatrice di proprietà](https://aka.ms/azure-tco-calculator) tooget un costo mensile hello di stima del servizio hello si è interessati. Ad esempio, una macchina virtuale di Windows A1 (VM) è stimato toocost $66.96 USD/mese nel calcolo ore se si lascia in esecuzione hello intero:
...
Per altre informazioni, vedere le [domande frequenti sui prezzi](https://azure.microsoft.com/pricing/faq/). O se si desidera tootalk tooa utente, chiamare 1-800-867-1389.
...
Esistono limiti predefiniti sottoscrizione tooeach per elementi quali hello numero di core CPU e indirizzi IP. Tenere presenti tali limiti. Per altre informazioni, vedere [Sottoscrizione di Azure e limiti, quote e vincoli dei servizi](../azure-subscription-service-limits.md). È possibile richiedere un aumento del limite di tooyour o di una quota da [come contattare il supporto](https://portal.azure.com/?#blade/Microsoft_Azure_Support/HelpAndSupportBlade).

## <a name="day-1-as-you-add-services"></a>Giorno 1: al momento dell'aggiunta di servizi

### <a name="review-hello-estimated-cost-in-hello-portal"></a>Costo stimato hello revisione nel portale di hello

In genere quando si aggiunge un servizio nel portale di Azure hello, è disponibile una visualizzazione che mostra un costo stimato simile al mese. Ad esempio, quando si sceglie di dimensioni hello della macchina virtuale Windows si vedere hello stimato costo mensile per ore di calcolo hello:
...
1. Visitare hello [pannello sottoscrizioni nel portale di Azure](https://portal.azure.com/#blade/Microsoft_Azure_Billing/SubscriptionsBlade).

2. Selezionare la sottoscrizione a cui si desidera toosee. È necessario solo un tooselect.

3. È consigliabile vedere scomposizione dei costi di hello e velocità nel Pannello di hello popup. Potrebbe non essere supportato per l'offerta (verrà visualizzato un avviso superiore hello). Attendere 24 ore dopo l'aggiunta di un servizio per toopopulate dati hello.
    
    ![Schermata di velocità e suddivisione in hello portale di Azure](./media/billing-getting-started/burn-rate.PNG)

4. È possibile dashboard tooyour di toopin hello view.

    ![Schermata di aggiunta di un dashboard toohello visualizzazione](./media/billing-getting-started/pin.PNG)

5. Fare clic su **analisi dei costi** in hello elenco toohello toosee sinistro hello costo dettagli in base alla risorsa.

    ![Schermata di hello. analisi costo nel portale di Azure](./media/billing-getting-started/cost-analysis.PNG)

6. È possibile filtrare la visualizzazione in base a diverse proprietà, come [tag](#tags), gruppo di risorse e intervallo di tempo. Fare clic su **applica** filtri hello tooconfirm e **scaricare** tooexport hello Visualizza tooa comma-separated CSV (valori) file.

7. Fare clic su una risorsa toosee spesa cronologia e la quantità è stata la determinazione dei costi è ogni giorno.
...
#### <a name="view-costs-for-all-your-subscriptions-in-hello-billing-blade"></a>Visualizza i costi per tutte le sottoscrizioni nel pannello fatturazione hello

Se si gestiscono più sottoscrizioni come hello amministratore dell'Account, è possibile visualizzare hello bill aggregazione quantità e la suddivisione per tutte le sottoscrizioni in hello [pannello fatturazione](https://portal.azure.com/#blade/Microsoft_Azure_Billing/BillingBlade). 
...
Dopo il primo periodo di fatturazione, è possibile scaricare la fattura in formato PDF (Portable Document Format) e i dettagli di utilizzo in formato CSV (Comma-Separated Values). È anche possibile acconsentire esplicitamente toohave fattura inviato tramite posta elettronica tooyou. Questi file della Guida toounderstand novità tooyou infine fatturati dopo imposte, sconti e riconoscimenti. Se non si dispone di una sottoscrizione di tooyour collegato metodo di pagamento, questi file potrebbero non essere disponibili per l'utente. Per ulteriori informazioni, vedere [come tooget la fatturazione di Azure della fattura e dati di utilizzo giornalieri](billing-download-azure-invoice-daily-usage-date.md) e [comprendere la fattura per Microsoft Azure](billing-understand-your-bill.md).

![Screenshot di una fattura in formato PDF](./media/billing-getting-started/invoice.png)

tag Hello impostate precedentemente visualizzata nei file CSV di hello dettagli utilizzo:
